...
def send_friend_request(request, id):
    ...
    return HttpResponseRedirect(f'/users/{to_user.profile.slug}')
...
def profile_view(request, slug):
    ...
    if p not in request.user.profile.friends.all():
        ...
        if len(FriendRequest.objects.filter(from_user=request.user, to_user=u)) == 1:
                button_status = 'friend_request_sent'

        # if we have received a friend request
        if len(FriendRequest.objects.filter(from_user=p.user, to_user=request.user)) == 1:
...
def my_profile(request):
    ...
    if p not in request.user.profile.friends.all():
        ...
        # if we have sent him a friend request
        if len(FriendRequest.objects.filter(from_user=request.user, to_user=you)) == 1:
                button_status = 'friend_request_sent'
...
def search_users(request):
    ...
    return render(request, "users/search_users.html", {'users': object_list})
